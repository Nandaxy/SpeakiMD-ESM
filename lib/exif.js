...
export async function writeExif(media, metadata) {
	...
	if (metadata && Object?.keys(metadata).length !== 0) {
		...
		const json = {
			'sticker-pack-id': metadata?.packId || `Nandaxy-${Date.now()}`,
			'sticker-pack-name': metadata?.packName || '',
			'sticker-pack-publisher': metadata?.packPublish || '',
			'android-app-store-link': metadata?.androidApp || 'https://github.com/Nandaxy/SpeakiMD-ESM',
			'ios-app-store-link': metadata?.iOSApp || 'https://github.com/Nandaxy/SpeakiMD-ESM',
